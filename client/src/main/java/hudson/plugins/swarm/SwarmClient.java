...
public class SwarmClient {
    ...
    protected void connect(Candidate target) throws InterruptedException {
        ...
        if(options.webSocket){
            VersionNumber jenkinsVersion = new VersionNumber(launcher.VERSION);
            VersionNumber requiredVersion = new VersionNumber("2.229");
            if(requiredVersion.isOlderThanOrEqualTo(jenkinsVersion)){
                args.add("-webSocket");
            }
            else{
                throw new RuntimeException(target.url + " does not support the web socket protocol");
            }
        }
