...
A single Jenkins controller currenly supports a single selection from a multitude of possible authentication providers.
...
* the Matrix-based security configuration with GitHub Authentication plugin supports user names as accounts, and organization names and `org*team` notation as groups; while it is reasonable to allow any members of the specified organization various permissions for the Jenkins jobs and other objects, an account used purely for agent connections does not need those (and should not inherit them by being an org member).
