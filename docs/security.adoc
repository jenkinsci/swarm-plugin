...
`-password`:: The Jenkins user API token or password. CloudBees CI users are advised to use service accounts to authenticate rather than personal access tokens.
...
`-noCrumb`:: Do not ask for a CSRF crumb. Recommended unless you are actually using a password, which is not recommended.
